...
const Media = require(path.join(__dirname, '..', 'schemas', 'media.schema'));
...
module.exports = exports = function open311(schema /*,options*/) {
   ...
   * @type {Function}
   */
  schema.statics.createFromOpen311Client = function(serviceRequest, done) {
    //refs
    ...
    async.waterfall(
      [
        function ensureServiceExist(next) {
          ...
          Service.findOne({ code: serviceRequest.service_code }).exec(function(
            error,
            service
          ) {
            if (!service) {
        ...
        function createServiceRequest(service, next) {
          ...
          serviceRequest.attachments = _.map(mediaUrls, function(mediaUrl) {
            const attachment = {
              url: mediaUrl,
              name: 'Attachement',
          ...
          serviceRequest = {
            ...
            reporter: {
              name: [serviceRequest.first_name, serviceRequest.last_name].join(
                ' '
              ),

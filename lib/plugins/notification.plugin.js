...
//utils
const Send = require(path.join(__dirname, '..', 'utils', 'send'));

module.exports = exports = function notification(schema /*, options*/ ) {
  //extend schema with notification fields
   ...
   * @type {Function}
   */
  schema.post('save', function sendOpenTicketToReporter(serviceRequest, next) {
    //refs
    const Message = mongoose.model('Message');

    //check if should send open ticket notification to reporter
    const sendTicket = !serviceRequest.wasOpenTicketSent &&
    ...
    if (sendTicket) {
      ...
      const template = getString(
        'SMS_TEMPLATE_TICKET_OPEN',
        'Dear customer. Your ticket # is {ticket} for {service} you have reported. Call {phone} for more support. Thanks.'
      );

      const body = parseTemplate(template, {
        ticket: serviceRequest.code,
        service: serviceRequest.service.name.en,
        phone: getString('PHONE', '255000000000'),
      });
   ...
   * @type {Function}
   */
  schema.post('save', function sendResolveTicketToReporter(
    serviceRequest,
    next
  ) {
    //refs
    ...
    const sendTicket =
      serviceRequest.resolvedAt &&
      !serviceRequest.wasResolveTicketSent &&
      !_.isEmpty(serviceRequest.reporter.phone);
    ...
    if (sendTicket) {
      //compile message to send to reporter
      const template = getString(
        'SMS_TEMPLATE_TICKET_RESOLVE',
        'Dear customer. Your issue # - {ticket} has been resolved. Call {phone} for confirmation. Thanks.'
      );

...
module.exports = exports = function pipeline(schema /*, options*/) {
  schema.statics.pipeline = function(criteria, done) {
    ...
    ServiceRequest.aggregated(criteria)
      .group({
        //1. count and group by status and operator
        _id: {
      ...
      .project({
        //3 stage: project full required documents
        _id: '$_id',
        party: '$_operator',
        status: '$_status',
        count: '$count',
      ...
      .exec(function(error, pipelines) {
        ...
        //min status weight
        let minStatusWeight = _.chain(pipelines)
          .map('status.weight')
          .min()
          .value();
        minStatusWeight =
        ...
        let operators = _.compact(
          _.map(pipelines, function(pipeline) {
            return pipeline.party ? pipeline.party._id.toString() : undefined;
          })
        );
        ...
        const totalReportedPerOperator = _.map(operators, function(_id) {
          ...
          const pipelinesPerOperator = _.filter(pipelines, function(pipeline) {
            if (pipeline.party) {
              return pipeline.party._id.toString() === _id;
            }
            return false;
          ...
          const totalPerOperator = _.sumBy(pipelinesPerOperator, 'count');

...
function normalizeTime(item) {
  ...
  item.averageResolveTime =
    item.averageResolveTime > 0
      ? item.averageResolveTime
      : -item.averageResolveTime;

  //parse to time units
  item.averageResolveTime = parseMs(item.averageResolveTime);

  //parse average attend time
  item.averageAttendTime =
    item.averageAttendTime > 0
      ? item.averageAttendTime
      : -item.averageAttendTime;
  /* jshint ignore:end */
  //parse to time units
  item.averageAttendTime = parseMs(item.averageAttendTime);
...
module.exports = exports = function performance(schema /*, options*/) {
  ...
  const JURISDICTION_FACET = [
    {
      //count and group by jurisdiction
      $group: {
  ...
  const SERVICE_GROUP_FACET = [
    {
      // count and group by service group
      $group: {
        _id: '$group._id',
        pending: { $sum: '$pending' },
        resolved: { $sum: '$resolved' },
  ...
  const WORKSPACE_FACET = [
    {
      // count and group by operator workspace
      $group: {
  ...
  const METHOD_FACET = [
    {
      // count and group by reporting method
      $group: {
  ...
  schema.statics.performance = function(criteria, done) {
    ...
    ServiceRequest.aggregated(criteria)
      ...
      .exec(function(error, performances) {
        ...
        //normalize overalls
        performances.overall = _.map(performances.overall, normalizeTime);
        performances.overall = _.first(performances.overall);

        //normalize jurisdictions
        performances.jurisdictions = _.map(
        ...
        //normalize services
        performances.services = _.map(performances.services, normalizeTime);

...
router.get(PATH_LIST, function getServiceRequests(request, response, next) {
  ...
  ServiceRequest.get(options, function onGetServiceRequests(error, results) {
    //forward error
    if (error) {
      next(error);
    }
...
router.post(PATH_LIST, function postServiceRequest(request, response, next) {
  ...
  ServiceRequest.post(body, function onPostServiceRequest(error, created) {
    //forward error
    if (error) {
      next(error);
    }
...
router.get(PATH_SINGLE, function getServiceRequest(request, response, next) {
  ...
  ServiceRequest.getById(options, function onGetServiceRequest(error, found) {
    //forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.patch(PATH_SINGLE, function patchServiceRequest(
  request,
  response,
  next
) {
  ...
  ServiceRequest.patch(id, patches, function onPatchServiceRequest(
    error,
    patched
  ) {
    //forward error
...
router.put(PATH_SINGLE, function putServiceRequest(request, response, next) {
  ...
  ServiceRequest.put(id, updates, function onPutServiceRequest(error, updated) {
    //forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.delete(PATH_SINGLE, function deleteServiceRequest(
  request,
  response,
  next
) {
  ...
  ServiceRequest.del(id, function onDeleteServiceRequest(error, deleted) {
    //forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.get(PATH_JURISDICTION, function getServiceRequests(
  request,
  response,
  next
) {
  ...
  const filter = jurisdiction ? { filter: { jurisdiction: jurisdiction } } : {}; //TODO support parent and no jurisdiction
  const options = _.merge({}, filter, request.mquery);

  ServiceRequest.get(options, function onGetServiceRequests(error, found) {
    //forward error
    if (error) {
      next(error);
    }
